...
# Byte-compilation is environment-dependent: pip compiles an installed wheel
# while other installers do not, and the cache filename encodes the interpreter
# version. Counting these would make the canonical digest differ per
# environment, breaking both the marker contract and no-op reinstall, so they
...
def _is_unmanaged(relative: str) -> bool:
    """Return whether one relative member is environment-generated bytecode."""
    return any(
        part in _UNMANAGED_DIRECTORIES for part in Path(relative).parts
    ) or relative.endswith(_UNMANAGED_SUFFIXES)
...
def _read_marker(destination: Path) -> dict[str, Any]:
    ...
    for relative, file_digest in managed.items():
        ...
        managed_relative = _validate_relative(relative)
        if _is_unmanaged(managed_relative):
            raise InvalidSkillMarkerError(
                "Comic Sol Skill marker claims environment-generated files; no files were changed"
            )
        normalized[managed_relative] = file_digest
