...
| Combined docs (`test_user_docs` + `test_release_docs` + `test_showcase_contract` + `web.tests.test_web_docs`) | pass — docs contract tests, 0 failures |
...
| Leakage scan (api_key/bearer/sk-/ghp_/Bearer/Set-Cookie/private-paths/loopback-endpoints/JSON-payload/authorization-header/cookie-secret shapes) on the new docs and submission | pass — 0 hits |
