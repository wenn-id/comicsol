...
from scripts import project_io as project_io_os
...
class BoundedReaderTests(unittest.TestCase):
    ...
    def test_non_regular_files_are_refused_before_reading(self):
        from types import SimpleNamespace
        import stat as stat_module

        with temporary_directory() as root:
            document = root / "pipe-like.bin"
            document.write_bytes(b"data")
            fake_stat = SimpleNamespace(st_mode=stat_module.S_IFIFO | 0o600, st_size=0)
            with mock.patch.object(project_io_os.os, "fstat", return_value=fake_stat):
                with self.assertRaises(InputResourceLimitError) as raised:
                    read_bytes_nofollow(document, max_bytes=16)
            self.assertIn("regular-file", str(raised.exception))

    def test_file_growth_between_size_check_and_read_is_caught(self):
        from types import SimpleNamespace
        import stat as stat_module

        with temporary_directory() as root:
            document = root / "growing.bin"
            document.write_bytes(b"z" * 64)
            lying_stat = SimpleNamespace(st_mode=stat_module.S_IFREG | 0o600, st_size=8)
            with mock.patch.object(project_io_os.os, "fstat", return_value=lying_stat):
                with self.assertRaises(InputResourceLimitError):
                    read_bytes_nofollow(document, max_bytes=16)
...
class RasterEncodedLimitTests(unittest.TestCase):
    ...
    def test_file_over_the_encoded_ceiling_is_rejected_before_decode(self):
        with temporary_directory() as root:
            ...
            self.assertIn("file size limit", str(raised.exception))
