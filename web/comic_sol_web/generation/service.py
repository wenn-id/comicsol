...
class GenerationService:
    ...
    def submit_staged_raster(
        ...
        try:
            ...
            # If a process died after WP3 committed but before this database row
            # advanced, WP3 reconciles its revision. Re-submit at that current
            # revision: its handoff acceptance is itself duplicate-safe.
            from comic_sol_web.engine_gateway import StaleProjectRevisionError

            if not isinstance(error, StaleProjectRevisionError):
                self._release_promotion(job_id, token)
                raise
            project_snapshot = self._projects.snapshot(principal, job.project_id)
            revision = cast(int, getattr(project_snapshot, "revision"))
            snapshot = self._projects.submit_raster(
                principal,
                job.project_id,
                revision,
